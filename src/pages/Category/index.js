...
import { ContentTop, FilterPopup, TableBlock, TableBodyBlock, TableHeadBlock } from '../../components'
...
const Category = React.memo(({
    ...
    onOpenCategory}) => {
        ...
        React.useEffect(() => {
            (async () => {
                try{
                    setIsFetching(true)
                    const respCategories = await axios.post(`/categories/search/`, { language: selectedLanguage, parent: null },  {
                        headers: {
    ...
    return <>
        <ContentTop onClick={onOpenCategory} searchValue={searchValue}  setSearchValue={setSearchValue} />
        <FilterPopup />
        {
            !isFetching 
            ?   <TableBlock>
                    ...
                    </TableHeadBlock>
                    {
                        ...
                        .map(item => 
                            <TableBodyBlock  key={item.id}>
                                ...
                                <TableCell>
                                    <Button variant="contained" color="secondary" onClick={() => onClickEditCategory(item)} className={classes.btnEdit}>
                                        Изменить
                                    </Button>
                                    <Button onClick={() => onRemoveItem(item.id)} variant="contained" color="primary" className={classes.btnRemove}>
                                        Удалить
                                    </Button>
                                </TableCell>
                            </TableBodyBlock>
